...
const KPI_TARGET_PERCENT = 80.0;
const TREND_STABLE_DELTA = 0.2;

const SECTION1_COMPETENCY_WEIGHTS = [
    'S1.1' => 0.05,
    'S1.2' => 0.10,
    'S1.3' => 0.10
];

const SECTION2_GROUP_WEIGHTS = [
...
function buildPeriodStaffScores(array $records, string $periodType): array
{
    ...
    foreach ($groups as $entry) {
        ...
        $results[] = [
            ...
            'performance_level' => classifyPerformance($finalPercent),
            'category_scores' => $categoryScores,
            'comments' => trim((string)$entry['comments']) !== '' ? $entry['comments'] : 'No supervisor comment recorded for this period.',
            'training' => trim((string)$entry['training']) !== '' ? $entry['training'] : 'No training recommendation recorded for this period.',
        ];
    }
...
function metricForRow(array $row, string $categoryFilter): array
{
    if ($categoryFilter === 'All Categories') {
        return [
            'score_5' => (float)$row['final_score_5'],
            'percentage' => (float)$row['final_percentage'],
        ];
    }
...
function buildDepartmentStats(array $staffLatest): array
{
    ...
    foreach ($grouped as $department => $rows) {
        ...
        $stats[] = [
            ...
            'risk_band' => $atRisk > 0 ? 'High' : ($avg < KPI_TARGET_PERCENT ? 'Moderate' : 'Low'),
        ];
    }
...
function buildRiskHistogram(array $staffLatest): array
{
    ...
    foreach ($staffLatest as $row) {
        ...
        elseif ($score < 60) $bins['40-59']++;
        elseif ($score < 70) $bins['60-69']++;
        elseif ($score < 80) $bins['70-79']++;
        else $bins['80-100']++;
    }
...
function buildTrendNarrative(array $timeline, string $periodType, string $categoryFilter): string
{
    ...
    $scope = $categoryFilter === 'All Categories' ? 'overall weighted KPI performance' : $categoryFilter . ' performance';
...
function buildComparisonTrendSeries(array $periodStaff, int $staff1Id, int $staff2Id, string $yearFilter): array
{
    ...
    foreach ($periodStaff as $row) {
        ...
        if ($id === $staff1Id) $byPeriod[$period]['staff1'] = $row['final_percentage'];
        if ($id === $staff2Id) $byPeriod[$period]['staff2'] = $row['final_percentage'];
    }
...
function extractHistoryForStaff(array $periodStaff, int $staffId): array
{
    ...
    foreach ($periodStaff as $row) {
        ...
        $rows[] = [
            'period' => $row['period'],
            'score_5' => round($row['final_score_5'], 2),
            'percentage' => $row['final_percentage'],
        ];
    }
    usort($rows, fn($a, $b) => strcmp($a['period'], $b['period']));
    return $rows;
